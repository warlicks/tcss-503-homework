...
class RedBlackBST:
    ...
    def insert_i(self, key, value):
        ...
        parent_node = self._insertion_search(insert_node)
    ...
    def _insertion_search(self, insert_node: RedBlackNode) -> RedBlackNode:
        ...
        The process follows a standard binary search process to find the node where
        the new data will be inserted. The standard search is described below.

        The method searches the tree by comparing key of the node being inserted
        to the current node. If the key is less than the current node key we
        "navigate" left. If the key is greater we "navigate" right.

        When navigate left or right we first check to see if their is a child node
        in that direction. If there is not a child node in that direction, the current
        node is returned. If there is a child node we move down the tree to that node
    ...
    def _put_i(self, parent_node: RedBlackNode, insertion_node: RedBlackNode):

        if insertion_node.key < parent_node.key:
            parent_node.left = insertion_node
            insertion_node.parent = parent_node
        elif insertion_node.key > parent_node.key:
            parent_node.right = insertion_node
            insertion_node.parent = parent_node
        elif insertion_node.key == parent_node.key:
            parent_node.value = insertion_node.value

    def _tree_correction(self, parent_node: RedBlackNode):
    ...
    def _put_r(self, node, key, value):
        ...
        if key < node.key:
            ...
            node.value = value
        # NODE HAD BEEN NONE b/c recursion, now it's the parent NODE. HOW????
...
def test_bst(bst):
    ...
    result = (
        "PASSED"
        if r == "one hundred"
        else f"Failed, expected 'one hundred', received {r}"
    )
